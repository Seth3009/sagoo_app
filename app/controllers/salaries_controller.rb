class SalariesController < ApplicationController
  ...
  def show
    ...
    @resto = Resto.find_by_employee_id(@employee)
    @attendances = Attendance.where('employee_id = ? AND att_month = ?', @employee, '2017-12-30')
    @total = 0.00
    @count = @attendances.sum(:day_count)
  end
  ...
  private
    ...
    def salary_params
      params.require(:salary).permit(:salary_period, :employee_id)
    end
end
